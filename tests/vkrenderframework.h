...
public:
    enum eTestEnFlags {
        eInvalidOffsetForBind,
        eBindNullBuffer,
        eDeleteInvalidBuffer,
    };

    cMemoryBuffer(VkDeviceObj *aVulkanDevice, VkBufferUsageFlags aBufferUsage,
                  VkDeviceSize &aBufferByteCount, const float *aClientData) :
       ...
       VulkanDevice(aVulkanDevice->device()) {
        ...
        bool pass = aVulkanDevice->phy().set_memory_type(memory_requirements.memoryTypeBits,
                                                         &memory_allocate_info, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
        if (!pass) {
    ...
    cMemoryBuffer(VkDeviceObj *aVulkanDevice, VkBufferUsageFlags aBufferUsage, eTestEnFlags aTestFlag) :
        AllocateCurrent(false),
        BoundCurrent(false),
        CreateCurrent(false),
        VulkanDevice(aVulkanDevice->device()) {
        ...
        if (eBindNullBuffer == aTestFlag) {
            ...
            vkBindBufferMemory(VulkanDevice, VulkanBuffer, VulkanMemory, 1); // NB: 1 is intentionally an invalid offset value
            BoundCurrent = true;
        }
    }
    ...
    void TestDoubleDestroy() {
        // Destroy the buffer but leave the flag set, which will cause the buffer to be
        //  destroyed again in the destructor.
        vkDestroyBuffer(VulkanDevice, VulkanBuffer, nullptr);
    }
...
class cVertices {
public:
    cVertices(VkDeviceObj *aVulkanDevice, unsigned aAttributeCount, unsigned aBindingCount,
              unsigned aByteStride, VkDeviceSize aVertexCount, const float *aVerticies) :
       BoundCurrent(false),
    ...
    bool AddVertexInputToPipe(VkPipelineObj &aPipelineObj) {
        if (!BoundCurrent || !VulkanMemoryBuffer.GetBufferCurrent()) {
            return false;
        }
    ...
    void TestDoubleDestroy() {
        VulkanMemoryBuffer.TestDoubleDestroy();
    }

    void TestInvalidOffsetWithBind(VkCommandBuffer aCommandBuffer) {
        VkDeviceSize offsetList[1];
        offsetList[0] = 1;

        vkCmdBindVertexBuffers(aCommandBuffer, BindId, 1, &VulkanMemoryBuffer.GetBuffer(), offsetList);
        BoundCurrent = true;
    }
