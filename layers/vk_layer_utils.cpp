...
//   o  By default, if neither a debug callback or vk_layer_settings.txt file is present
//
// At layer initialization time, default logging callbacks are created to output layer error messages.
// Any settings found in a vk_layer_settings.txt file will override any default settings. If an application
// later registers a debug callback function and no vk_layer_settings.txt file was located, the default
// logging callbacks will be removed. Any settings in a vk_layer_settings.txt file will override
// those in an application's debug callback function.
void layer_debug_actions(debug_report_data *report_data, std::vector<VkDebugReportCallbackEXT> &logging_callback,
    ...
    bool default_layer_callback = debug_action & VK_DBG_LAYER_ACTION_DEFAULT;
